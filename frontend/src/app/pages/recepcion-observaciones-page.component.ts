...
import { ObservacionWorkStatus, RecepcionesService } from '../services/recepciones.service';

@Component({
  selector: 'app-recepcion-observaciones-page',
  standalone: true,
    ...
    <main class="simple-shell">
      <section class="header">
        <div>
          ...
          <h1>Registrar observación de trabajo</h1>
        </div>
      ...
      <form class="card form-grid" *ngIf="!accessDenied" (ngSubmit)="submit()">
        ...
        <label class="field">
          ...
          <select [(ngModel)]="form.work_status" name="work_status">
            <option *ngFor="let option of workStatusOptions" [value]="option">{{ option }}</option>
          </select>
        </label>

        <p class="loading" *ngIf="isLoading">Registrando observación...</p>
        <p class="error" *ngIf="errorMessage">{{ errorMessage }}</p>

        <div class="actions">
          <button class="button primary" type="submit" [disabled]="isLoading">Guardar observación</button>
        </div>
      </form>
    </main>
  `,
  styles: [`
    ...
    .field select, .field textarea { border: 1px solid #c8d5e6; border-radius: .85rem; padding: .8rem .95rem; font: inherit; }
...
export class RecepcionObservacionesPageComponent {
  ...
  submit(): void {
    ...
    this.recepcionesService
      .crearObservacion(this.recepcionId, {
        observation_text: this.form.observation_text.trim(),
  ...
  private resolveErrorMessage(error: HttpErrorResponse): string {
    ...
    if (error.status === 403) {
      return 'No tienes permisos para registrar observaciones.';
    }
