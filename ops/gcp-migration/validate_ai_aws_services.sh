...
BASE_URL_AI="${BASE_URL_AI:-http://54.221.156.220:8010}"
TMP_DIR="$(mktemp -d /tmp/ai-aws-validation-XXXXXX)"
EMERGENCIA_ID="${EMERGENCIA_ID:-vm2-aws-$(date +%s)}"
...
cat > "${TEXT_FILE}" <<EOF
Validacion academica VM2 -> AWS IA
Emergencia: ${EMERGENCIA_ID}
Origen: Google Cloud / VM2
EOF

# Archivo binario minimo para el endpoint de vision.
printf '\377\330\377\340AWSJPEG\377\331' > "${IMAGE_FILE}"
...
print_section "1. health AWS IA"
health_status="$(curl -sS -o "${health_response}" -w '%{http_code}' "${BASE_URL_AI}/health")"
print_response "${health_status}" "${health_response}"
ensure_status "${health_status}" "200" "health AWS IA"
ensure_json_key "${health_response}" '.status == "ok"' "health AWS IA"

print_section "2. upload S3 AWS"
s3_status="$(curl_form "${BASE_URL_AI}/aws-s3/upload-evidencia" "${s3_response}" \
...
print_section "3. registro DynamoDB AWS"
...
dynamo_status="$(curl_json "POST" "${BASE_URL_AI}/dynamodb/evidencia" "${dynamo_payload}" "${dynamo_response}")"
...
print_section "4. registro Blockchain AWS"
blockchain_payload="$(jq -nc \
  --arg emergencia_id "${EMERGENCIA_ID}" \
  --arg evento "Validacion academica VM2->AWS IA" \
  --arg timestamp "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
  '{emergencia_id:$emergencia_id,evento:$evento,timestamp:$timestamp}')"
blockchain_status="$(curl_json "POST" "${BASE_URL_AI}/blockchain/registrar" "${blockchain_payload}" "${blockchain_response}")"
...
print_section "5. Deep Learning AWS"
vision_status="$(curl_form "${BASE_URL_AI}/ai/deep-learning/vision" "${vision_response}" \
  -F "emergencia_id=${EMERGENCIA_ID}" \
...
print_section "6. n8n webhook AWS"
n8n_payload="$(jq -nc \
  --arg emergencia_id "${EMERGENCIA_ID}" \
  --arg descripcion "Validacion academica VM2->AWS IA desde Google Cloud" \
  '{emergencia_id:$emergencia_id,descripcion:$descripcion}')"
n8n_status="$(curl_json "POST" "${BASE_URL_AI}/automation/n8n/webhook" "${n8n_payload}" "${n8n_response}")"
...
echo "VALIDACION_AI_AWS_SERVICES=OK"
